...
static std::vector<std::uint64_t> pack_words_lsb_first(const std::string &bits)
{
...
static void run_case_and_compare(const std::string &bits, std::mt19937_64 &rng, size_t ops_per_case, uint64_t seed)
{
    ...
    for (size_t q = 0; q < ops_per_case; ++q)
    {
        ...
        { // range_min_query_pos
            ...
            EXPECT_EQ(a, b) << trace_header << "op=range_min_query_pos(" << i << "," << j << ")";
        }
        break;
        ...
        { // range_min_query_val
            ...
            EXPECT_EQ(a, b) << trace_header << "op=range_min_query_val(" << i << "," << j << ")";
        }
        break;
        ...
        { // minselect
            ...
            EXPECT_EQ(a, b) << trace_header << "op=minselect(" << i << "," << j << "," << k << ")";
        }
        break;
        ...
        { // range_max_query_pos
            ...
            EXPECT_EQ(a, b) << trace_header << "op=range_max_query_pos(" << i << "," << j << ")";
        }
        break;
        ...
        { // range_max_query_val
            ...
            EXPECT_EQ(a, b) << trace_header << "op=range_max_query_val(" << i << "," << j << ")";
        }
        break;
...
protected:
    ...
    void SetUp() override
    {
        ...
        seed = choose_seed();
        rng.seed(seed);
        std::cerr << "[ RmMRandomTest ] seed=" << seed
                  << " CASES=" << L.CASES
                  << " OPS=" << L.OPS_PER_CASE
                  << " MAX_N=" << L.MAX_N
                  << "\n";
...
TEST_F(RmMRandomTest, ShortInputs)
{
    for (size_t n = 1; n <= 8; ++n)
    {
        ...
        for (size_t mask = 0; mask < total; ++mask)
        {
            ...
            {
                SCOPED_TRACE(::testing::Message() << "short-inputs:string bits=" << bits);
                run_case_and_compare(bits, rng, /*ops_per_case=*/200, /*seed=*/0xC0FFEEull);
            }
            {
                SCOPED_TRACE(::testing::Message() << "short-inputs:words  bits=" << bits);
                run_case_and_compare(bits, rng, /*ops_per_case=*/200, /*seed=*/0xC0FFEEull);
            }
        }
    }
...
static void run_case_string_vs_words(const std::string &bits, std::mt19937_64 &rng)
{
    const size_t N = bits.size();
    ...
    for (int t = 0; t < 128; ++t)
    {
        ...
        case 9:
        {
            if (N == 0)
                break;
            EXPECT_EQ(rm_s.range_min_query_pos(i, j), rm_w.range_min_query_pos(i, j));
            break;
        }
        case 10:
        {
            if (N == 0)
                break;
            EXPECT_EQ(rm_s.range_max_query_pos(i, j), rm_w.range_max_query_pos(i, j));
            break;
        }
        }
    }
