...
/**
 * @brief Find first equal lane in a fixed 256-bit block.
 * @details Returns lane index in [0..15] or @p npos if not found.
 */
static inline size_t find_forward_equal_i16_avx2(const int16_t* block16,
                                                 const int16_t& target,
                                                 const size_t& npos) noexcept {
#ifdef PIXIE_AVX2_SUPPORT
  const __m256i vtarget = _mm256_set1_epi16(target);
  const __m256i vdata =
      _mm256_loadu_si256(reinterpret_cast<const __m256i*>(block16));
  const unsigned int mask =
      _mm256_movemask_epi8(_mm256_cmpeq_epi16(vdata, vtarget));
  if (!mask) {
    return npos;
  }
...
/**
 * @brief Find last equal lane in a fixed 256-bit block.
 * @details Returns lane index in [0..15] or @p npos if not found.
 */
static inline size_t find_backward_equal_i16_avx2(const int16_t* block16,
                                                  const int16_t& target,
                                                  const size_t& npos) noexcept {
#ifdef PIXIE_AVX2_SUPPORT
  const __m256i vtarget = _mm256_set1_epi16(target);
  ...
  return (31u - std::countl_zero(mask)) >> 1;
#else
  for (int i = 15; i >= 0; --i) {
    if (block16[i] == target) {
      return i;
...
/**
 * @brief Find first equal lane in a fixed 512-bit block.
 * @details Returns lane index in [0..31] or @p npos if not found.
 */
static inline size_t find_forward_equal_i16_avx512(
    const int16_t* block32,
    const int16_t& target,
    const size_t& npos) noexcept {
#if defined(PIXIE_AVX512_SUPPORT)
  const __m512i vtarget = _mm512_set1_epi16(target);
  const __m512i vdata =
      _mm512_loadu_si512(reinterpret_cast<const void*>(block32));
  const __mmask32 mask = _mm512_cmpeq_epi16_mask(vdata, vtarget);
  if (!mask) {
    return npos;
  }
  return std::countr_zero((uint32_t)mask);
#else
  for (int i = 0; i < 32; ++i) {
    if (block32[i] == target) {
      return i;
...
/**
 * @brief Find last equal lane in a fixed 512-bit block.
 * @details Returns lane index in [0..31] or @p npos if not found.
 */
static inline size_t find_backward_equal_i16_avx512(
    const int16_t* block32,
    const int16_t& target,
    const size_t& npos) noexcept {
#if defined(PIXIE_AVX512_SUPPORT)
  ...
  return 31u - std::countl_zero((uint32_t)mask);
#else
  for (int i = 31; i >= 0; --i) {
    if (block32[i] == target) {
      return i;
