...
class RmMTree {
 ...
 public:
  ...
  size_t select1(size_t k) const {
    ...
    return select1_in_block(base,
                            std::min(base + segment_size_bits[v], num_bits), k);
  }
  ...
  size_t select10(size_t k) const {
    ...
    return select10_in_block(
        base, std::min(base + segment_size_bits[ind], num_bits), k);
  }
  ...
  size_t fwdsearch(const size_t& i, const int& d) const {
    ...
    if (blk + 1 < leaf_count) {
      ...
      for (size_t j = 0; j < cnt; ++j) {
        ...
        if (node_min_prefix_excess[v] <= need &&
            need <= node_max_prefix_excess[v]) {
  ...
  size_t bwdsearch(const size_t& i, const int& d) const {
    ...
    size_t base = Lb;
    while (v > 1) {
      if (v & 1) {                 // v is the right child
        const size_t sib = v ^ 1;  // left sibling
        const size_t border =
            base;  // right border of the sibling (== start(v))
        const int need_node =
            need + node_total_excess[sib];  // target in coordinates relative to
                                            // the start of sib
        const bool allow_rb = (border != i);  // j must be < i

        // try inside the sibling, but return only if a position is found
        if (need_node == 0 || (node_min_prefix_excess[sib] <= need_node &&
                               need_node <= node_max_prefix_excess[sib])) {
          const size_t ans = descend_bwd(sib, border - segment_size_bits[sib],
                                         need_node, border, allow_rb);
          if (ans != npos) {
            return ans;
          }
        }
  ...
  size_t range_min_query_pos(const size_t& i, const size_t& j) const {
    ...
    if (i <= end_first) {
      ...
      pref = (int64_t)rank1_in_block(i, end_first + 1) * 2 -
             int64_t(end_first + 1 - i);
      best_val = mn_first;
      best_pos = first_pos;
      chosen_node = 0;
    }
    ...
    return descend_first_min(chosen_node, best_val - pref_at_choice,
                             node_base(chosen_node));
  }
  ...
  size_t range_max_query_pos(const size_t& i, const size_t& j) const {
    ...
    return descend_first_max(chosen_node, best_val - pref_at_choice,
                             node_base(chosen_node));
  }
 ...
 private:
  ...
  size_t rank1_in_block(const size_t& Lb, const size_t& Rb) const noexcept {
    ...
    if (w_l == w_r) {
      const std::uint64_t mask =
          ((off_r == 0) ? 0 : ((std::uint64_t(1) << off_r) - 1)) &
          (~std::uint64_t(0) << off_l);
    ...
    if (off_r) {
      cnt +=
          (size_t)std::popcount(bits[w_r] & ((std::uint64_t(1) << off_r) - 1));
    }
    return cnt;
  }
  ...
  size_t select10_in_block(const size_t& Lb,
    ...
    const auto select_in_masked_slice = [&](const std::uint64_t& slice,
                                            const int& len,
                                            const size_t& kk) noexcept -> int {
      if (len <= 1) {
        return -1;
      }
  ...
  size_t descend_fwd(size_t v, int need, size_t base) const noexcept {
    while (v < first_leaf_index) {
      ...
      if (node_min_prefix_excess[Lc] <= need &&
          need <= node_max_prefix_excess[Lc]) {
        v = Lc;
      } else {
  ...
  size_t descend_bwd(size_t v,
    ...
    while (v < first_leaf_index) {
      ...
      if (node_min_prefix_excess[Rc] <= need_r &&
          need_r <= node_max_prefix_excess[Rc]) {
        const size_t ans = descend_bwd(Rc, base + segment_size_bits[Lc], need_r,
                                       right_border, allow_rb);
        if (ans != npos) {
          return ans;
        }
      }
      ...
      if (need == node_total_excess[Lc] &&
          (j_border < right_border || allow_rb)) {
        return j_border;
      }
  ...
  size_t descend_qth_min(size_t v,
    ...
    return qth_min_in_block(
        base, std::min(base + segment_size_bits[v], num_bits) - 1, q);
  }
  ...
  size_t select1_in_block(const size_t& Lb,
    ...
    const std::uint64_t mask_l =
        (off_l ? (~std::uint64_t(0) << off_l) : ~std::uint64_t(0));
    if (w_l == w_r) {
      const std::uint64_t w = bits[w_l] & mask_l &
                              ((Rb & 63) ? ((std::uint64_t(1) << (Rb & 63)) - 1)
                                         : ~std::uint64_t(0));
   ...
   */
  static inline size_t nodeslots_for(const size_t& Nbits,
   ...
   */
  static inline float overhead_for(const size_t& Nbits,
                                   const size_t& Bpow2) noexcept {
   ...
   */
  inline void first_min_value_pos8(size_t l,
                                   const size_t& r,
                                   int& mn_out,
   ...
   */
  inline void first_max_value_pos8(size_t l,
                                   const size_t& r,
                                   int& mx_out,
                                   size_t& first_pos) const noexcept {
    const auto& T = LUT8();
  ...
  void build(const size_t& leaf_block_bits, const float& max_overhead) {
    ...
    for (size_t v = first_leaf_index - 1; v >= 1; --v) {
      ...
      if (has_l && !has_r) {
        ...
        node_min_count[v] =
            (node_min_prefix_excess[Lc] == node_min_prefix_excess[v]
                 ? node_min_count[Lc]
                 : 0) +
            (m_r == node_min_prefix_excess[v] ? node_min_count[Rc] : 0);
        node_pattern10_count[v] =
            node_pattern10_count[Lc] + node_pattern10_count[Rc] +
            ((node_last_bit[Lc] == 1 && node_first_bit[Rc] == 0) ? 1u : 0u);
        node_first_bit[v] = node_first_bit[Lc];
        node_last_bit[v] = node_last_bit[Rc];
      }
